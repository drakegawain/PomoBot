...
async def message_time_to_rest(ctx:nextcord.Interaction, session):
  message = "\nTime to rest. \nYou have {} minutes."  .format(int(session.rest_time_global/60))
  embed.add_field(name = "Rest", value = message)
  await ctx.send(embed=embed)
  return

async def message_time_to_study(ctx:nextcord.Interaction, session):
  message = "Time to study/work. You have {} minutes."  .format(int(session.rest_time_global/60))
